        ...
        ((dma_buffer[1] - dma_buffer[0]) < 2200))
    {
        ...
        if (calibration_required)
        {
            if (!high_calibration_set)
            {
                ...
                {
                    servo_high_threshold = ((7 * servo_high_threshold +
                                             (dma_buffer[1] - dma_buffer[0])) >>
                                            3);
                    if (high_calibration_counts > 50)
                    {
                        servo_high_threshold = servo_high_threshold - 25;
                        eepromBuffer.servo.high_threshold =
                            (servo_high_threshold - 1750) / 2;
            ...
            if (high_calibration_set)
            {
                if (dma_buffer[1] - dma_buffer[0] < 1250)
                {
                    ...
                    servo_low_threshold = ((7 * servo_low_threshold +
                                            (dma_buffer[1] - dma_buffer[0])) >>
                                           3);
                }
                if (low_calibration_counts > 75)
                {
                    ...
                    eepromBuffer.servo.low_threshold =
                        (servo_low_threshold - 750) / 2;
                    calibration_required = 0;
                    saveEEpromSettings();
        ...
        if (!armed)
        {
            if (dshot && (average_count < 8) && (zero_input_count > 5))
            {
                ...
                average_packet_length =
                    average_packet_length +
                    (uint16_t)(dma_buffer[31] - dma_buffer[0]);
                if (average_count == 8)
                {
