...
void           computeDshotDMA()
{
    ...
    if ((dshot_frametime > dshot_frametime_low) &&
        (dshot_frametime < dshot_frametime_high))
    {
        signaltimeout = 0;
        for (int i = 0; i < 16; i++)
        {
            // note that dma_buffer[] is uint32_t, we cast the difference to
            // uint16_t to handle timer wrap correctly
            const uint16_t pdiff =
                dma_buffer[(i << 1) + 1] - dma_buffer[(i << 1)];
        ...
        if (calcCRC == checkCRC)
        {
            ...
            if (programming_mode > 0)
            {
                ...
                {
                    if (tocheck == 37)
                    { // commit new values to eeprom. must use save settings to
                      // make permanent.
...
void make_dshot_package(uint16_t com_time)
{
    ...
    {
        // Only send extended telemetry if last frame wasn't extended. This
        // ensures eRPM interleaving.
    ...
    else
    {
        ...
        //	calculate shift amount for data in format eee mmm mmm mmm, first 1
        //found
    ...
    dshot_full_number = (dshot_full_number << 4) |
    ...
    gcrnumber = gcr_encode_table[(dshot_full_number >> 12)]
                ...
                | gcr_encode_table[(((1 << 4) - 1) & (dshot_full_number >>
                                                      0))]; // last four digits
// GCR RLL encode 20 to 21bit output
    ...
    { // each digit in gcrnumber
        gcr[buffer_padding + 20 - i + 1] =
            ((((gcrnumber & 1 << i)) >> i) ^
             (gcr[buffer_padding + 20 - i] >> 7))
            << 7; // exclusive ored with number before it multiplied by 64 to
                  // match output timer.
    }
    gcr[buffer_padding] = 0;
#endif
