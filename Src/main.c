...
void loadEEpromSettings()
{
    ...
    if (eepromBuffer.eeprom_version < EEPROM_VERSION)
    {
        ...
        eepromBuffer.absolute_voltage_cutoff =
            10; // voltage level 1 to 100 in 0.5v increments
    ...
    // eepromBuffer.advance_level can either be set to 0-3 with config tools
    // less than 1.90 or 10-42 with 1.90 or above
    if (eepromBuffer.advance_level > 42 ||
        (eepromBuffer.advance_level < 10 && eepromBuffer.advance_level > 3))
    {
        temp_advance = 16;
    }
    if (eepromBuffer.advance_level < 4)
    { // old format needs to be converted to 0-32 range
        temp_advance               = (eepromBuffer.advance_level << 3);
        eepromBuffer.advance_level = temp_advance + 10;
    }
    if (eepromBuffer.advance_level < 43 && eepromBuffer.advance_level > 9)
    { // new format subtract 10 from advance
    ...
    if (eepromBuffer.minimum_duty_cycle < 51 &&
        eepromBuffer.minimum_duty_cycle > 0)
    {
    ...
    { // these commands weren't introduced until eeprom version 1.
        ...
        servo_high_threshold =
            (eepromBuffer.servo.high_threshold * 2) +
            1750; // anything above this point considered 2000 (max)
        ...
        if (eepromBuffer.driving_brake_strength == 0 ||
            eepromBuffer.driving_brake_strength > 9)
        { // motor brake 1-9
        ...
        if (eepromBuffer.driving_brake_strength < 10)
        {
            ...
            throttle_max_at_low_rpm =
                throttle_max_at_low_rpm + dead_time_override;
            startup_max_duty_cycle =
                startup_max_duty_cycle + dead_time_override;
#ifdef STMICRO
        ...
        if (eepromBuffer.limits.current > 0 &&
...
void commutate()
{
    ...
    if (step == 1 || step == 4)
    {
        WRITE_REG(RPM_PULSE_PORT->ODR,
                  READ_REG(RPM_PULSE_PORT->ODR) ^ RPM_PULSE_PIN);
    }
#endif
}
...
void PeriodElapsedCallback()
{
    ...
    {
        advance = (commutation_interval * temp_advance) >>
...
void interruptRoutine()
{
    ...
    //        (zero_crosses < 500)) { // should be impossible, desync?exit
    //        anyway
    //           return;
    //        }
    //        stuckcounter++; // stuck at 100 interrupts before the main loop
    //        happens
    //                        // again.
    //        if (stuckcounter > 100) {
    //            maskPhaseInterrupts();
...
void setInput()
{
        ...
        if (dshot == 0)
        {
            if (eepromBuffer.rc_car_reverse)
            {
                if (newinput > (1000 + (servo_dead_band << 1)))
                {
                    ...
                    if (prop_brake_active == 0)
                    {
                        return_to_center = 0;
                        adjusted_input =
                            map(newinput, 1000 + (servo_dead_band << 1), 2000,
                                47, 2047);
                    }
                }
                if (newinput < (1000 - (servo_dead_band << 1)))
                {
                    ...
                    if (prop_brake_active == 0)
                    {
                        return_to_center = 0;
                        adjusted_input =
                            map(newinput, 0, 1000 - (servo_dead_band << 1),
                                2047, 47);
                    }
                }
                if (newinput >= (1000 - (servo_dead_band << 1)) &&
            ...
            else
            {
                if (newinput > (1000 + (servo_dead_band << 1)))
                {
                    ...
                    adjusted_input =
                        map(newinput, 1000 + (servo_dead_band << 1), 2000, 47,
                            2047);
                }
                if (newinput < (1000 - (servo_dead_band << 1)))
                {
                    ...
                    adjusted_input = map(
                        newinput, 0, 1000 - (servo_dead_band << 1), 2047, 47);
                }
    ...
    if (!stepper_sine && armed)
    {
        if (input >= 47 + (80 * eepromBuffer.use_sine_start))
        {
            ...
            {
                duty_cycle_setpoint =
                    map(input, 47, 2047, minimum_duty_cycle, 2000);
            }
                ...
                if (eepromBuffer.rc_car_reverse && prop_brake_active)
                {
                    ...
                    if (dshot == 0)
                        prop_brake_duty_cycle =
                            (getAbsDif(1000, newinput) + 1000);
                    if (dshot)
                    {
                        if (newinput <= 1047 && newinput > 47)
                            prop_brake_duty_cycle = ((newinput - 48) * 2 + 47) -
                                                    reversing_dead_band;
                        if (newinput > 1047)
                            prop_brake_duty_cycle =
                                ((newinput - 1048) * 2 + 47) -
                ...
                if (!running)
                {
                    ...
                    if (eepromBuffer.brake_on_stop > 0)
                    {
                        if (!eepromBuffer.use_sine_start)
                        {
                            ...
                            {
                                prop_brake_duty_cycle =
                                    eepromBuffer.drag_brake_strength * 200;
                                if (prop_brake_duty_cycle >= (1999))
                                {
                                    fullBrake();
                                }
...
#else
                            // todo add proportional braking for pwm/enable
                            // style bridge.
#endif
                        }
                    }
        ...
        if (!prop_brake_active)
        {
            if (input >= 47 &&
                (zero_crosses <
                 (uint32_t)(30 >> eepromBuffer.stall_protection)))
            {
...
{ // 20khz as of 2.00 to be renamed
    ...
    if (eepromBuffer.telemetry_on_interval)
    {
        ...
        if (telem_ms_count >
            ((telemetry_interval_ms - 1 + eepromBuffer.telemetry_on_interval) *
             20))
        {
            // telemetry_on_interval = 1 is a boolean, but it can also be 2 or
            // more to indicate an identifier by making the interval just
            // slightly different with an unique identifier, we can guarantee
            // that many ESCs can communicate on just one signal there will be
            // some collisions but not as many as if two ESCs always tried to
            // talk at once.
            send_telemetry = 1;
            telem_ms_count = 0;
        }
    ...
    if (!stepper_sine)
    {
        ...
        else
        {
            ...
            {
                adjusted_duty_cycle =
                    tim1_arr - ((prop_brake_duty_cycle * tim1_arr) / 2000);
            }
            else
            {
                if ((eepromBuffer.brake_on_stop == 2) && armed)
                { // require arming for active brake
                    comStep(2);
                    adjusted_duty_cycle =
                        DEAD_TIME +
                        ((eepromBuffer.active_brake_power * tim1_arr) / 2000) *
                            10;
                }
                else
                {
...
{ // only used in polling mode, blocking routine.
    ...
    advance =
        (temp_advance * commutation_interval) >> 6; //   7.5 degree increments
...
int main(void)
{
    ...
    if (!eepromBuffer.comp_pwm)
    {
        eepromBuffer.use_sine_start =
            0; // sine start requires complementary pwm.
    }
    ...
    while (1)
    {
        ...
        { // uses range defined by pwm frequency setting
        ...
        if (dshot_telemetry &&
            (commutation_interval > DSHOT_PRIORITY_THRESHOLD))
        {
        ...
        if (send_telemetry)
        {
            ...
            makeTelemPackage((int8_t)degrees_celsius, battery_voltage,
                             actual_current, (uint16_t)(consumed_current >> 16),
                             e_rpm);
            send_telem_DMA(10);
            send_telemetry = 0;
#endif
        ...
        { // for adc and telemetry set adc counter at 1khz loop rate
#if defined(STMICRO)
            ADC_DMA_Callback();
...
#endif
            converted_degrees = __LL_ADC_CALC_TEMPERATURE(
                3300, ADC_raw_temp, LL_ADC_RESOLUTION_12B);
#endif
#ifdef MCU_GDE23
            ADC_DMA_Callback();
            // converted_degrees = (1.43 - ADC_raw_temp * 3.3 / 4096) * 1000
            // / 4.3 + 25;
            converted_degrees =
            ...
            if (eepromBuffer.low_voltage_cut_off == 1)
            {
                ...
                else
                {
                    ...
                    { // if set low cutoff has happened, require power cycle to
                      // reset
                        low_voltage_count = 0;
                    }
                }
            ...
            { // 10 second wait before cut-off for low voltage
        ...
        if (stepper_sine == 0)
        {
            ...
            k_erpm = e_rpm / 10; // ecom time is time for one electrical
                                 // revolution in microseconds
            ...
            { // some hardware doesn't need this, its on
              ...
              // in the very low end a little.
                duty_cycle_maximum =
                    map(k_erpm, low_rpm_level, high_rpm_level,
                        throttle_max_at_low_rpm,
                        throttle_max_at_high_rpm); // for more performance lower
                                                   // the high_rpm_level, set to
                                                   // a consvervative number in
                                                   // source.
            }
            else
            {
        ...
        { // stepper sine
            ...
            if (input > 48 && armed)
            {
                ...
                { // sine wave stepper
                    ...
                    advanceincrement();
                    step_delay =
                        map(input, 48, 120, 7000 / eepromBuffer.motor_poles,
                            810 / eepromBuffer.motor_poles);
                    delayMicros(step_delay);
                    e_rpm = 600 / step_delay; // in hundreds so 33 e_rpm is 3300
                                              // actual erpm
                }
                else
                {
